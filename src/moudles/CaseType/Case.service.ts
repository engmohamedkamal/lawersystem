...
class usersService {
    ...
    createCaseType = async (req: Request, res: Response, next: NextFunction) =>{
      ...
      const create = await CaseTypeModel.findOne({ name })

      if(create) throw new AppError("CaseType already exist" , 409)

      const CaseType = await CaseTypeModel.create({name , createdBy : req.user?._id})
    ...
    getCaseTypes = async (req: Request, res: Response, next: NextFunction) => {
        const caseTypes = await CaseTypeModel.find({ isActive: true }).sort({ createdAt: -1 })
