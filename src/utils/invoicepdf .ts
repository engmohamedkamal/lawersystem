...
// ─── Browser singleton — مش بنفتح browser جديد كل request ─────────────────
...
// ─── cache اللوجو عشان مش نحمله كل مرة ────────────────────────────────────
...
// ─── الفونت embedded بدل Google Fonts ────────────────────────────────────
const FONT_STYLE = `
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  body { font-family: Arial, sans-serif; background: #fff; color: #0E1A2B; font-size: 13px; }
  ...
  .warning-banner { background: #FEE2E2; border-right: 4px solid #DC2626; padding: 10px 20px; margin: 12px 32px 0; border-radius: 6px; display: flex; align-items: center; gap: 10px; }
...
// ─── HTML builder مشترك ────────────────────────────────────────────────────
const buildInvoiceHTML = (invoice: any, settings: any, logoSrc: string, index = 0) => {
...
<div class="invoice-page ${index > 0 ? "page-break" : ""}">
  ...
  <div class="cards">
    <div class="card">
      ...
      <div class="card-info">${invoice.client?.phone ?? ""}<br/>${invoice.client?.email ?? ""}<br/>${invoice.client?.address ?? ""}</div>
    </div>
    ${invoice.legalCase ? `
    <div class="card">
      <div class="card-title">القضية</div>
      <div class="card-name">${invoice.legalCase?.caseNumber ?? "-"}</div>
      <div class="card-info">الحالة: ${invoice.legalCase?.status ?? "-"}<br/>المحكمة: ${invoice.legalCase?.court ?? "-"}<br/>المدينة: ${invoice.legalCase?.city ?? "-"}</div>
    </div>` : ""}
  </div>
  ...
  <div class="table-wrap">
    <table>
      ...
      <tbody>
        ${invoice.items?.map((item: any) => `<tr><td>${item.description}</td><td>${fmt(item.amount)}</td></tr>`).join("") ?? ""}
      </tbody>
    </table>
...
// ─── توليد PDF فاتورة واحدة ────────────────────────────────────────────────
export const generateInvoicePDF = async (invoice: any, settings: any): Promise<Buffer> => {
    const logoSrc = settings?.logo ? (await fetchImageAsBase64(settings.logo)) ?? "" : ""

    const html = `<!DOCTYPE html><html lang="ar" dir="rtl"><head><meta charset="UTF-8"/>${FONT_STYLE}</head><body>
        ${buildInvoiceHTML(invoice, settings, logoSrc)}
    </body></html>`
    ...
    try {
        // domcontentloaded أسرع بكتير من networkidle0
...
export const generateAllInvoicesPDF = async (invoices: any[], settings: any): Promise<Buffer> => {
    ...
    const logoSrc     = settings?.logo ? (await fetchImageAsBase64(settings.logo)) ?? "" : ""
    const invoicesHTML = invoices.map((inv, i) => buildInvoiceHTML(inv, settings, logoSrc, i)).join("")
